...
public class VectorTextFile {
	 ...
	 * word in the initial text file, a pair consisting of the word itself and
	 * the number of times it appears in the file. This array is sorted in the
	 * order of the words. All letters in all words are lower-case, and all
	...
	protected void ParseFile()
	{
		...
		// Check if list is sorted successfully.
		VerifySort(wordList);
	 ...
	 *         in the array. Each word appears the same number of times in the
	 *         list as it does in the original file. All letters in all words
	...
	{		
		...
		try{
			...
			// Read in the file, one character at a time. For each character,
			// normalize it by removing punctuation and capitalization.
			for (int i = 0; i < size; i++)
			{
				...
				{
					// In this case, we add a space to the string. Note: only
					// add a space if the previous character is not also a
					// space. This prevents adding two spaces in a row.
	...
	private static String[] SplitString(String textFile)
	{
		...
		// Note: the length of the string is an overestimate of the number of
		// words in the String. As a result, we allocate too much space, wasting
		// memory. It would be better to allocate space more efficiently.
	...
	{	
		...
		if ((wordList == null) || (wordList.length == 0)) {
			throw new IllegalArgumentException("Failed InsertionSort: no words to sort.");
		}
		...
		// The goal of this iteration is to find the proper slot for the string
		...
		for (maxSorted = 0; wordList[maxSorted + 1] != null; maxSorted++) {
			...
			// We first find where SortString fits in array [1..iMaxSorted + 1]
			index = maxSorted + 1;
			...
			// Now that we have found where SortString goes, move it into place.
	...
	private static WordCountPair[] CountWordFrequencies(String[] wordList)
	{
		...
		// words in the wordList. Notice that this is an inefficient use of
		// space, as countedWords will likely be much smaller than wordList.
		...
		for (int i = 1; wordList[i] != null; i++)
		{
			...
			if (wordList[i].equals(word)) {
				...
				// Since we have already sorted all the words, the same words
				// will exist continuously. In other words, if the next word is
				// not the same, the last word will not appear anymore.
