...
public class EvenBetterVectorTextFile extends BetterVectorTextFile {
	...
	// m_FileWordCount has a count of the number of words. Also, we require
	// that: 0 <= begin <= end < m_FileWordCount.
	 ...
	 * Note: The list of words ends at the first null string.
	 * 
	 * Properties: On completion, the words in wordList between Begin and End
	 * are sorted alphabetically.
	 * 
	 * Methodology: The sorting is accomplished via 'merge sort.' This sorting
	 * method runs in O(n log n) time, instead of the slower `insertion sort'
	 * used in the parent class.
	 * 
	 * @param wordList
	 *            array of words to be sorted
	 * @param begin
	 *            index to begin sorting, inclusive
	 * @param end
	 *            index to stop sorting, inclusive
	 */
	private void MergeSortWords(String[] wordList, int begin, int end) {
		...
		// We now divide the list into two parts, each 1/2 the size of the
		// initial list. The first list is from [Begin..Middle-1] and the second
		// list is from [Middle..End].
		//
		// Notice: Division by two automatically rounds to an integer, since
		// this is a key feature of integer division.
	...
	// Output: None
	// Assumptions: m_WordList holds a non-empty set of words, and
	// m_FileWordCount has a count of the number of words. In addition, we
	// require that 0 <= Begin <= Middle <= End < m_FileWordCount.
	...
	private void Merge(String[] wordList, int begin, int middle, int end) {
		...
		for (int i = 0; i < numWords; i++) {
			...
			// Otherwise, if we have exhausted all the words in BList, then we
			// copy a word from AList
