...
public class MergeSortReverse {
	 ...
	 *   should be inverse to the original order.
	 * 
	 * @param arr
	 *            Array of positive double to be sorted
	 */
	public static void sort(double[] arr) {
		MergeSortReverse sorter = new MergeSortReverse();
		sorter.mergeSortReverse(arr, 0, arr.length - 1);
	}

	private void mergeSortReverse(double[] arr, int start, int end) {
		if (start == end) {
			return;
		} else {
			int mid = (start + end) / 2;

			mergeSortReverse(arr, start, mid);
			mergeSortReverse(arr, mid + 1, end);
